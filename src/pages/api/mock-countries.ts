...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  const [status, json] = requestHelper(req);
  res.status(status).json(json);
}

function requestHelper(req: NextApiRequest): [status: number, json: Object] {
  if (process.env.NODE_ENV === 'production') {
    return [400, 'Bad request'];
  }

  if (req.query['cca3'] === undefined) {
    return [200, mockCountries];
  }
  const country = mockCountries.find((country) => country.cca3 === req.query['cca3']);
  if (!country) {
    return [400, 'Bad request'];
  }
  return [200, country];
